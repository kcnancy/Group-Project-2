...
const meanings = {
  upright: [
    ...
    {
      name:
        "Ace of Pentacles symbolizes a new beginning in your financial situation and an opportunity to deepen your security."
    },
    {
      name:
    ...
    {
      name:
        "Three of Pentacles symbolizes your work, especially where team work or listening to clients/customers are concerned. This is a positive card which indicates job satisfaction and taking pride in your work, but you must put in the effort to see results."
    },
    {
      name:
    ...
    {
      name:
        "World is the final Major Arcana card and represents fulfillment and successful completion of a cycle. You know your place in the world, and your life lessons have made you smart and accomplished. The World shows up when the world is ready for you and want what you have to offer."
    }
  ],
  reversed: [
    //CUPS
...
function createDeck() {
  ...
  for (let a0 = 0; a0 < 4; a0++) {
    ...
    for (let a1 = 1; a1 < 15; a1++) {
      ...
      const name = _.kebabCase(displayName);
...
function randGen() {
  const cardsLeft = deckArr.length;
  const randInt = Math.floor(Math.random() * cardsLeft);
  const randNum = deckArr[randInt];
...
function pastPresentFuture() {
  ...
  const percentRevs = $("#range").html();

  for (let b = 1; b <= 3; b++) {
    //$("#rev-" + b).html("");
    const rand = randGen();
    const randInvert = Math.floor(Math.random() * 101);
